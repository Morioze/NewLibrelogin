...
public class LoginTryListener<P, S> {
    ...
    private final RateLimiter<String> loginRateLimiter;
    ...
    public LoginTryListener(AuthenticLibreLogin<P, S> libreLogin) {
        ...
        var rateLimitMs = plugin.getConfiguration().get(ConfigurationKeys.LOGIN_RATE_LIMIT_MS);
        this.loginRateLimiter = rateLimitMs > 0 ? new RateLimiter<>(rateLimitMs, TimeUnit.MILLISECONDS) : null;
    ...
    private void onWrongPassword(WrongPasswordEvent<P, S> wrongPasswordEvent) {
        ...
        if (player != null && loginRateLimiter != null && loginRateLimiter.tryAndLimit(plugin.getPlatformHandle().getIP(player))) {
            plugin.getPlatformHandle().kick(player, plugin.getMessages().getMessage(MessageKeys.KICK_RATE_LIMITED.key()));
            return;
        }
