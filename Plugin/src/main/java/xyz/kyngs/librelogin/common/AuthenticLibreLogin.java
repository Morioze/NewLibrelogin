...
public abstract class AuthenticLibreLogin<P, S> implements LibreLoginPlugin<P, S> {
    ...
    protected void enable() {
        ...
        startAutoUpdate(configuration.get(AUTO_UPDATE), configuration.get(AUTO_UPDATE_URL), configuration.get(AUTO_UPDATE_INTERVAL));
    ...
    protected void startAutoUpdate(boolean enabled, String url, long intervalSeconds) {
        if (!enabled || url == null || url.isBlank()) return;
        autoUpdateUrl = url;
        long intervalMillis = Math.max(intervalSeconds, 60) * 1000L;
        repeat(this::autoUpdate, intervalMillis, intervalMillis);
        logger.info("Auto-update enabled: checking " + autoUpdateUrl + " every " + (intervalMillis / 1000) + " seconds.");
    }

    private void autoUpdate() {
        try {
            logger.info("Checking for LibreLogin updates...");
            var connection = new URL(autoUpdateUrl).openConnection();
            connection.setConnectTimeout(10_000);
            connection.setReadTimeout(30_000);
            connection.setRequestProperty("User-Agent", "LibreLogin AutoUpdater");

            var temp = Files.createTempFile("librelogin-update", ".jar");
            try (var in = connection.getInputStream()) {
                Files.copy(in, temp, StandardCopyOption.REPLACE_EXISTING);
            }

            var current = getCurrentJar();
            if (current == null) {
                Files.deleteIfExists(temp);
                return;
            }

            if (sha256(temp).equals(sha256(current))) {
                logger.info("LibreLogin is already up to date.");
                Files.deleteIfExists(temp);
                return;
            }

            var backup = current.resolveSibling(current.getFileName() + ".old");
            try {
                Files.move(current, backup, StandardCopyOption.REPLACE_EXISTING);
            } catch (IOException e) {
                var staged = current.resolveSibling(current.getFileName() + ".new");
                Files.move(temp, staged, StandardCopyOption.REPLACE_EXISTING);
                logger.error("The running LibreLogin jar is locked and cannot be replaced (this happens on Windows). The newest jar was saved as " + staged.getFileName() + ". Stop the server, replace the jar with it, and start again. Download: " + autoUpdateUrl);
                return;
            }
            try {
                Files.move(temp, current, StandardCopyOption.REPLACE_EXISTING);
            } catch (IOException e) {
                Files.move(backup, current, StandardCopyOption.REPLACE_EXISTING);
                throw e;
            }

            logger.warn("LibreLogin has been updated to the newest version. The old jar was kept as " + backup.getFileName());
            restartForUpdate();
        } catch (Exception e) {
            logger.warn("Failed to auto-update LibreLogin", e);
        }
    }
    ...
    protected void restartForUpdate() {
        logger.warn("Please restart the server/proxy to apply the LibreLogin update.");
    }
