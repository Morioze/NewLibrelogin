...
public class AuthenticAuthorizationProvider<P, S> extends AuthenticHandler<P, S> implements AuthorizationProvider<P> {
    ...
    public void authorize(User user, P player, AuthenticatedEvent.AuthenticationReason reason) {
        if (plugin.getConfiguration().get(ConfigurationKeys.DEBUG)) {
            plugin.getLogger().debug("authorize() called for %s, reason=%s, tracked-as-unauthorized=%s".formatted(user.getLastNickname(), reason, String.valueOf(unAuthorized.containsKey(player))));
        }
    ...
    public void startTracking(User user, P player) {
        ...
        if (plugin.getConfiguration().get(ConfigurationKeys.DEBUG)) {
            plugin.getLogger().debug("startTracking() for %s, was-already-tracked=%s, unAuthorized.size=%d".formatted(user.getLastNickname(), String.valueOf(unAuthorized.containsKey(player)), unAuthorized.size()));
        }
    ...
    public void notifyUnauthorized() {
        ...
        unAuthorized.forEach((player, registered) -> {
            ...
            if (plugin.getConfiguration().get(ConfigurationKeys.DEBUG)) {
                plugin.getLogger().debug("notifyUnauthorized() re-sending login prompt to %s (registered=%s)".formatted(String.valueOf(player), registered));
            }
