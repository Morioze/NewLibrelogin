...
public class BungeeCordLibreLogin extends AuthenticLibreLogin<ProxiedPlayer, ServerInfo> {
    ...
    @Nullable
    private ProxyAuthServer proxyAuthServer;
    @Nullable
    private CancellableTask proxyAuthPruneTask;
    ...
    @Nullable
    public ProxyAuthServer getProxyAuthServer() {
        return proxyAuthServer;
    }

    @Override
    protected void restartForUpdate() {
        getLogger().warn("!! LibreLogin has been updated !! Restart the proxy to apply it. If your launcher restarts the proxy, execute /end.");
    }
    ...
    protected void enable() {
        ...
        int verifyPort = getConfiguration().get(ConfigurationKeys.PROXY_AUTH_VERIFY_PORT);
        if (verifyPort > 0) {
            proxyAuthServer = new ProxyAuthServer(getLogger());
            if (proxyAuthServer.start(verifyPort)) {
                proxyAuthPruneTask = repeat(proxyAuthServer::prune, 10 * 60 * 1000, 10 * 60 * 1000);
            } else {
                proxyAuthServer = null;
            }
        }
    }
    ...
    protected void disable() {
        if (proxyAuthServer != null) {
            if (proxyAuthPruneTask != null) proxyAuthPruneTask.cancel();
            proxyAuthServer.stop();
            proxyAuthServer = null;
        }
        super.disable();
        if (adventure != null) {
