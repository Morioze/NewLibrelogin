...
public class VelocityListeners extends AuthenticListeners<VelocityLibreLogin, Player, RegisteredServer> {
    ...
    public void onServerConnected(ServerConnectedEvent event) {
        ...
        if (!limboServers.contains(serverName)) return;
        ...
        plugin.delay(() -> {
            ...
            if (player.getCurrentServer().isEmpty() || !limboServers.contains(player.getCurrentServer().get().getServerInfo().getName()))
                return;
            ...
            try {
                ...
                if (lobby != null) player.createConnectionRequest(lobby).connect();
